...
java -server $JAVA_OPTS $JAVA_OPTS_SCRIPT $ORIENTDB_SETTINGS -Djava.util.logging.config.file="$LOG_FILE" -Dorientdb.config.file="$CONFIG_FILE" -Dorientdb.www.path="$WWW_PATH" -Dorientdb.build.number="306" -cp "$ORIENTDB_HOME/lib/orientdb-server-1.4.1.jar:$ORIENTDB_HOME/lib/*" com.orientechnologies.orient.server.OServerMain
